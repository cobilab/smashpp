...
class application {
 ...
 private:
  ...
  void prepare_data();
  void convert_to_seq(std::string, std::string, FileType const&);
  void show_info(std::unique_ptr<Param>&);
  void make_ir_consistent(std::unique_ptr<FCM>&);
...
void application::show_info(std::unique_ptr<Param>& par) {
...
void application::make_ir_consistent(std::unique_ptr<FCM>& models) {
...
void application::write_pos_file() {
